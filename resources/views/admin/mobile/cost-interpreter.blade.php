    ...
    <div id="list-data">
            ...
            <div class="modal-dialog char-h-mobile">
                <div class="modal-content" >
                    ...
                    <div class="page-separator">
                        ...
                        <div class="page-separator">
                            <div class="page-separator__text">
                            {{ trans("label.graph") }}( {{ trans("label.interpreter_cost") }})
                            </div>
                        </div>
                        <div class="card">
                            <div class="col-lg-12">
            ...
            <div class="modal-dialog char-h-mobile">
                <div class="modal-content" >
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row">
                            <div class="form-group col-lg-12">
                                <label class="form-label">{{ trans('label.bank_status') }}</label>
                            </div>
                            <div class="form-group col-lg-12">
                                <input type="checkbox" id="checkedMoveStatus1" value="0" v-model="checkedCostInterpreterBankStatus">
                                <label for="checkedMoveStatus1">{{ trans('label.bank_status_0') }}</label>
                            </div>
                            <div class="form-group col-lg-12">
                                <input type="checkbox" id="checkedMoveStatus0" value="1" v-model="checkedCostInterpreterBankStatus">
                                <label for="checkedMoveStatus0">{{ trans('label.bank_status_1') }}</label>
                            </div>
                        </div>
                        <div class="page-separator-line"></div>
                        <div class="row">
        ...
        <div class="modal fade" id="myModalChartMonth">
            <div class="modal-dialog char-h-mobile">
                <div class="modal-content" >
                    <div class="modal-header">
                        <h4 class="modal-title">{{ trans('label.interpreter_cost_chart') }}</h4>
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                    </div>
                    <div class="modal-body">
        ...
        <div class="modal fade" id="myModalChartYear">
            <div class="modal-dialog char-h-mobile">
                <div class="modal-content" >
                    <div class="modal-header">
                        <h4 class="modal-title">{{ trans('label.interpreter_cost_year_chart') }}</h4>
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                    </div>
                    <div class="modal-body">
        ...
        <div class="row">
            <div class="bodyButtonTopMobile fullWidthMobile">
                <a type="button" class="btn btn-outline-secondary3 pdfButtonBg" target="_blank" :href="'/admin/cost-interpreter-pdf?' + conditionSearch">
                        ...
                        <tbody class="list" id="search">
                            <tr v-for="item in sortedProducts">
                                ...
                                <a type="button" class="btn btn-outline-secondary" target="_blank" :href="'/admin/projectview/' + item.id">
                                    ...
                                    {{ trans('label.interpreter_cost') }}=<span class="moneyCol" >(( new Intl.NumberFormat('ja-JP', { style: 'currency', currency: 'JPY',currencyDisplay: 'name' }).format(item.sumPhPhienDich) ))</span><br>
                                    </span>
                                </td>
                            </tr>
                        </tbody>
...
new Vue({
    ...
    data: {
        ...
        checkedCostStatus: [0,1],
        checkedCostInterpreterBankStatus: [],
		thang_phien_dich: '',
        name_kh: '',
        ctv_pd: '',
        ctv_sale: '',
        ...
        sortedProducts: function(){
            return this.list.sort((p1,p2) => {
                ...
                if (this.sortBy == 'ctv_sales_list' || this.sortBy == 'ctv_list') {
                    ...
                    if(p1[this.sortBy][0]['name'] > p2[this.sortBy][0]['name']) return 1 * modifier;
                    return 0;
                } else if (this.sortBy == 'bank_date') {
                    if(p1["ctv_list"].length == 0 && p2["ctv_list"].length > 0) return -1 * modifier; 
                    if(p1["ctv_list"].length > 0 && p2["ctv_list"].length == 0) return 1 * modifier;
                    if(p1["ctv_list"].length == 0 && p2["ctv_list"].length == 0) return 0;

                    if(p1["ctv_list"][0]['ngay_chuyen_khoan'] < p2["ctv_list"][0]['ngay_chuyen_khoan']) return -1 * modifier; 
                    if(p1["ctv_list"][0]['ngay_chuyen_khoan'] > p2["ctv_list"][0]['ngay_chuyen_khoan']) return 1 * modifier;
                    return 0;
                } else if (this.sortBy == 'bank_name') {
                    if(p1["ctv_list"].length == 0 && p2["ctv_list"].length > 0) return -1 * modifier; 
                    if(p1["ctv_list"].length > 0 && p2["ctv_list"].length == 0) return 1 * modifier;
                    if(p1["ctv_list"].length == 0 && p2["ctv_list"].length == 0) return 0;

                    if(p1["ctv_list"][0]['bank_id'] < p2["ctv_list"][0]['bank_id']) return -1 * modifier; 
                    if(p1["ctv_list"][0]['bank_id'] > p2["ctv_list"][0]['bank_id']) return 1 * modifier;
                    return 0;
                } else if (this.sortBy == 'bank_status') {
                    if(p1["ctv_list"].length == 0 && p2["ctv_list"].length > 0) return -1 * modifier; 
                    if(p1["ctv_list"].length > 0 && p2["ctv_list"].length == 0) return 1 * modifier;
                    if(p1["ctv_list"].length == 0 && p2["ctv_list"].length == 0) return 0;

                    if(p1["ctv_list"][0]['status'] < p2["ctv_list"][0]['status']) return -1 * modifier; 
                    if(p1["ctv_list"][0]['status'] > p2["ctv_list"][0]['status']) return 1 * modifier;
                    return 0;
                } else {
    ...
    methods: {
        ...
        notPaySearch: function() {
            this.clearSearchCondition();

            var today = new Date();
            today.setDate(today.getDate()-1);
            this.ngay_phien_dich_to = this.formatDate(today);

            this.checkedCostInterpreterBankStatus = [0];
        ...
        notPaySearchPDF: function() {
            this.notPaySearch();
            location.href = "/admin/cost-interpreter-notpay-pdf?notpay=1&"+this.conditionSearch;
        },
        ...
        loadChartMonth() {
            ...
            setTimeout(function(){ 
                ...
                series:[
                    { label: '{{ trans("label.interpreter_cost") }}' ,
        ...
        resetSearch() {
            ...
            this.checkedCostStatus = [0,1];
            this.checkedCostInterpreterBankStatus = [];
        ...
        onLoadPagination() {
            ...
            if (this.checkedCostStatus.length > 0) {
                conditionSearch += '&check_cost_interpreter_status=' + this.checkedCostStatus.join();
            }
            if (this.checkedCostInterpreterBankStatus.length > 0) {
                conditionSearch += '&check_cost_interpreter_bank_status=' + this.checkedCostInterpreterBankStatus.join();
            }
            if (this.sortName != '') {
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.count > 0) {
                        ...
                        that.sumCost = data.sumCostInterpreter;
                        that.sumPhPhienDich = data.sumCostInterpreter;
                        that.sumPhiGiaoThong = data.sumCostMoveFee;
                        that.sumSendInterpreter = data.sumSendInterpreter;
                        that.yearReportInterpreter = data.yearReportInterpreter;
                        that.everyYearReportInterpreter = data.everyYearReportInterpreter;
                    } else {
                        that.count = 0;
                        that.sumCount = data.count;
                        that.list = [];
                        that.sumCost = data.sumCostInterpreter;
                        that.yearReportInterpreter = [];
                        that.everyYearReportInterpreter = [];
                    }
                    that.loadingTable = 0;
                    let pageArr = [];
                    if (that.page - 2 > 0) {
