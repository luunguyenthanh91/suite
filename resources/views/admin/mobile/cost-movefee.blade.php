    ...
    <div id="list-data">
            ...
            <div class="modal-dialog char-h-mobile">
                <div class="modal-content" >
                    ...
                    <div class="page-separator">
                                ...
                                <div class="row" style="margin-top:15px;">
                                    <div class="form-group col-lg-12">
                                        <a class="dropdown-item" target="_blank" @click="lastMonthProject()">
                                            <i class="fas fa-search-plus"></i><span class="labelButtonDropMenu">{{ trans('label.lastmonth_cost_move') }}</span>
                                        </a>  
                                        <a class="dropdown-item" target="_blank" @click="thisMonthProject()">
                                            <i class="fas fa-search-plus"></i><span class="labelButtonDropMenu">{{ trans('label.thismonth_cost_move') }}</span>
                                        </a>  
                                        <a class="dropdown-item" target="_blank" @click="thisYearProject()">
                                            <i class="fas fa-search-plus"></i><span class="labelButtonDropMenu">{{ trans('label.thisyear_cost_move') }}</span>
                                        </a> 
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="page-separator">
                            <div class="page-separator__text">
                            {{ trans("label.graph") }}( {{ trans("label.move_fee") }})
                            </div>
                        </div>
                        <div class="card">
                            <div class="col-lg-12">
            ...
            <div class="modal-dialog char-h-mobile">
                <div class="modal-content" >
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row">
                            <div class="form-group col-lg-12">
                                <label class="form-label">{{ trans('label.receipt') }}</label>
                            </div>
                            <div class="form-group col-lg-12">
                                <input type="checkbox" id="checkedReceiptStatuss1" value="0" v-model="checkedReceiptStatus">
                                <label for="checkedReceiptStatus1">{{ trans('label.not') }}</label>
                            </div>
                            <div class="form-group col-lg-12">
                                <input type="checkbox" id="checkedReceiptStatus0" value="1" v-model="checkedReceiptStatus">
                                <label for="checkedReceiptStatus0">{{ trans('label.yes') }}</label>
                            </div>
                        </div>
                        <div class="page-separator-line"></div>
                        <div class="row">
        ...
        <div class="modal fade" id="myModalChartMonth">
            <div class="modal-dialog char-h-mobile">
                <div class="modal-content" >
                    <div class="modal-header">
                        <h4 class="modal-title">{{ trans('label.sale_cost_movefee_chart') }}</h4>
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                    </div>
                    <div class="modal-body">
        ...
        <div class="modal fade" id="myModalChartYear">
            <div class="modal-dialog char-h-mobile">
                <div class="modal-content" >
                    <div class="modal-header">
                        <h4 class="modal-title">{{ trans('label.sale_cost_movefee_year_chart') }}</h4>
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                    </div>
                    <div class="modal-body">
                        ...
                        <tbody class="list" id="search">
                            <tr v-for="item in sortedProducts">
                                ...
                                <a type="button" class="btn btn-outline-secondary" target="_blank" :href="'/admin/projectview/' + item.id">
                                    ...
                                    {{ trans('label.move_fee') }}=<span class="moneyCol" >(( new Intl.NumberFormat('ja-JP', { style: 'currency', currency: 'JPY',currencyDisplay: 'name' }).format(item.sumPhiGiaoThong) ))</span><br>
                                    </span><br>
                                    <a v-if="item.sumFileHoaDon" target="_blank" :href="'' + item.sumFileHoaDon">
                                            (( item.sumFileHoaDon ))
                                            <br>
                                    </a>
                                    <a v-if="item.sumFileHoaDon" type="button" class="btn btn-outline-secondary3" style="background:#f38434" target="_blank" :href="'/admin/move-fee-receipt-pdf/' + item.id">
                                        <i class="fas fa-file-pdf"><span class="labelButton">{{ trans('label.show_receipt') }}</span></i>
                                    </a> 
                                    <div v-if="!item.sumFileHoaDon">{{ trans('label.not') }}</div>
                                </td>
                            </tr>
                        </tbody>
                    </table>                       
    ...
    methods: {
        ...
        onLoadPagination() {
            ...
            if (this.checkedCostStatus.length > 0) {
                conditionSearch += '&check_cost_movefee_status=' + this.checkedCostStatus.join();
            }
            if (this.checkedReceiptStatus.length > 0) {
                conditionSearch += '&check_receipt_movefee_status=' + this.checkedReceiptStatus.join();
            }
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.count > 0) {
                        ...
                        that.sumCost = data.sumCostMoveFee;
                        that.yearReportMoveFee = data.yearReportMoveFee;
                        that.everyYearReportMoveFee = data.everyYearReportMoveFee;
                    } else {
                        that.count = 0;
                        that.sumCount = data.count;
                        that.list = [];
                        that.sumCost = data.sumCostMoveFee;
                        that.yearReportMoveFee = [];
                        that.everyYearReportMoveFee = [];
                    }
                    that.loadingTable = 0;
                    let pageArr = [];
                    if (that.page - 2 > 0) {
