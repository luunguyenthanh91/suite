    ...
    <div id="list-data">
        <div class="row">
            <div class="bodyButtonTopMobile fullWidthMobile">
                <a type="button" class="btn btn-outline-secondary3" style="background:green" href="/admin/customer-update/{{$id}}">
                            ...
                            <div class="row">     
                                <table class="table thead-border-top-0 table-nowrap table-mobile propertiesTables">   
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if ( @$data->status == '' ||  @$data->status == 1)
                                                <span>{{ trans('label.cus_status_1') }}</span>
                                            @endif
                                            @if ( @$data->status == 2 )
                                                <span>{{ trans('label.cus_status_2') }}</span>
                                            @endif
                                            @if ( @$data->status == 3 )
                                                <span>{{ trans('label.cus_status_3') }}</span>
                                            @endif
                                            @if ( @$data->status == 4 )
                                                <span>{{ trans('label.cus_status_4') }}</span>
                                            @endif
                                            @if ( @$data->status == 5 )
                                                <span>{{ trans('label.cus_status_5') }}</span>
                                            @endif
                                            @if ( @$data->status == 6 )
                                                <span>{{ trans('label.cus_status_6') }}</span>
                                            @endif
                                        </td>
                                    </tr> 
                                    <tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                        <div>(( parseAddr('{{$data->address}}') ))</div>
                                        <a target="_blank" href="http://maps.google.com/maps?q={{@$data->address}}" id="link-map-address" type="button" class="btn btn-outline-secondary googleMapButton">
                            ...
                            <div class="row">
                                <table class="table mb-0 thead-border-top-0 table-nowrap style-group-add">
                                    <tbody class="list" id="search">
                                        <tr v-for="item in sortedSaleList">
                                            <td>
                                                <a  target="_blank" :href="'/admin/partner-sale-view/' + item.id" class="btn btn-outline-secondary" style="text-transform: uppercase;text-align:left;">
                        ...
                        <div class="tab-pane" id="detailtab3">
                            ...
                            <div class="row">
                                <table class="table thead-border-top-0 table-nowrap mb-0">
                                    ...
                                    <tbody class="list" id="search">
                                        <tr v-for="(item,index) in sortedProducts">
...
new Vue({
    ...
    computed: {
        ...
        sortedSaleList: function(){
            return this.listCustomerSale.sort((p1,p2) => {
    ...
    methods: {
        ...
        onLoadPagination() {
            ...
            $.ajax({
                type: 'GET',
                url: "{{route('admin.getListPartnerSale')}}?customer_id=" + this.itemId,
                success: function(data) {
                    that.listCustomerSale = data.data;                  
                },
                error: function(xhr, textStatus, error) {
                    Swal.fire({
            ...
            let conditionSearch = '';
            conditionSearch += '&customer_id=' + that.itemId;
            conditionSearch += '&showcount=' + that.showCount; 
            conditionSearch += '&sortname=ngay_pd';
            conditionSearch += '&sorttype=DESC';    
            that.conditionSearch = conditionSearch;   
            $.ajax({
                type: 'GET',
                url: "{{route('admin.getListProject')}}?page=" + that.page  + conditionSearch ,
                success: function(data) {
                    if (data.count > 0) {
