    ...
    <div id="list-data">
        <div class="bodyButtonTop">
            <a type="button" class="btn btn-outline-secondary3" style="background:#FF8C00" target="_blank" href="/admin/worksheet-pdf/{{$id}}">
                <i class="fa fa-file-pdf"><span class="labelButton">{{ trans('label.worksheetsche_pdf') }}</span></i>
            </a>
            @if (Auth::guard('admin')->user()->id == 1 || $data->status == 0 || $data->status == 1)
            <a type="button" class="btn btn-outline-secondary3" style="background:green" href="/admin/worksheetsche-update/{{$id}}">
                    ...
                    <div class="col-lg-auto cardTopViewArea">
                        ...
                        {{ trans('label.dep') }}: {{@$data->employee_depname}}<br>
                         {{ trans('label.name') }}: {{@$data->employee_name}} ({{@$data->user_id}})<br>
                          {{ trans('label.work_day_count_sche') }}: (( daycount ))<br>
                          {{ trans('label.work_time_count_sche') }}: (( worktimecount ))
                        </div>
                    </div>
                        ...
                        <div class="row no-gutters" role="tablist">
                            ...
                            <div class="col-auto border-left border-right">
                                <a data-toggle="tab" role="tab" aria-selected="false" class="dashboard-area-tabs__tab card-body d-flex flex-row align-items-center justify-content-start tab_click" id="tab3">
                                    <span class="flex d-flex flex-column">
                                        <strong class="card-title">{{ trans('label.worksheetsche') }}</strong>
                                    </span>
                                </a>
                            </div>
                        </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-12">
                                    ...
                                    <div class="card">
                                        <table class="table thead-border-top-0 table-nowrap table-mobile propertiesTables">
                                        <tr>
                                                <td>{{ trans('label.worksheetsche_id') }}</td>
                                                <td>
                                                    {{@$data->id}}
                                                </td>
                                            </tr>
                                            ...
                                            <tbody class="list" id="search">
                                                <tr v-for="item in sortedProducts">
                                                    ...
                                                    <td class="textAlignCenter">
                                                        <span v-if="item.starttime">{{ trans('label.do_work_day') }}</span>
                                                    </td>
                                                    <td class="textAlignCenter">
                                                        (( item.starttime ))
                                                    </td>
                ...
                Swal.fire({
                    ...
                    $.ajax({
                        ...
                        success: function(res) {
                            ...
                            location.href = "/admin/worksheetsche-view/" + _i;
                        },
                        error: function(xhr, textStatus, error) {
                            Swal.fire({
                                title: "エラーが発生しました!",
                ...
                Swal.fire({
                    ...
                    $.ajax({
                        ...
                        success: function(res) {
                            ...
                            location.href = "/admin/worksheetsche";
                        },
                        error: function(xhr, textStatus, error) {
                            Swal.fire({
                                title: "Có lỗi dữ liệu nhập vào!",
